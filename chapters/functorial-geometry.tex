...
%
  ...
  Mathieu Anel. We may consider classifying topos $\brk{\mathbf{LocalLoc}}$ of
  the theory of \emph{local localizations}, a subtopos of the cotensor
  $\vvmathbb{2}\times\brk{\Ring}$ (the classifier of ring maps);
...
\begin{namespace}{partition-of-unity}
  \begin{para}[Partition of unity]\nslabel{para}
    ...
    for any generating Zariski local cover
    $\Mor{\Coprod{i}\Yo{A\brk{\Inv{f_i}}}}{\Yo{A}}$, the following diagram
    ...
    each $A\brk{\Inv{a_j}}$. First we observe that each $a_j$ can be massaged into a
