...
\NewDocumentCommand\PrFlt{}{\mathbf{PrFlt}}
...
\begin{para}\label{para:spec-underlying-space}
  ...
  topoi $\Mor{\brk{\PrFlt}}{\brk{\Ring}}$. In the category of sets, prime
  ...
  ring). The fiber of $\brk{\PrFlt}$ over a ring $A$ is then the classifying
...
\begin{para}\label{para:localization-geom-mor}
  %
  We will exhibit a morphism of topoi $\Mor{\brk{\PrFlt}}{\brk{\LocRing}}$
  ...
  $\SET\brk{\PrFlt}$ to the local ring $U\brk{\Inv{S}}$.
  %
\end{para}
...
\begin{para}
  ...
  refer to $\Sh{\Spec{A}} = \SET\brk{\PrFlt\prn{A}}$ as the \emph{little Zariski logos} of $A$.
  %
\end{para}
