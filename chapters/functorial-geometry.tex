...
\NewDocumentCommand\RingLoc{}{\mathbf{Ring}_{\mathrm{Loc}}}
...
become easier and less technical.
...
\begin{para}
  ...
  $\RingLoc$ for the category of local rings and local ring homomorphisms.
  %
\end{para}
...
\begin{para}
  ...
  they both have classifying topoi, respectively $\brk{\Ring},\brk{\RingLoc}$.
...
\begin{para}
  ...
  In particular, $\SET\brk{\RingLoc}$ is a localization of $\SET\brk{\Ring}$
...
\begin{para}\label{para:spec-underlying-space}
  ...
  ring $A$ is then classifying topos of prime ideals of $A$, the underlying space of the \emph{spectrum} of $A$.
\end{para}

\begin{para}\label{para:localization-geom-mor}
  %
  We will exhibit a morphism of topoi $\Mor{\brk{\PrimeIdl}}{\brk{\RingLoc}}$
  ...
  $\SET\brk{\PrimeIdl}$ to the local ringcr$U_{\frk{p}}$.
  %
\end{para}
...
\begin{para}
  ...
  \[
    \begin{tikzpicture}[diagram]
      ...
      \node (nee) [above = 2.5cm of ne] {$\RingLoc$};
      \path[->] (ne) edge (nee);
      \path[->,bend left = 20] (nw) edge node [sloped,above] {$\Str{\Spec{A}}$} (nee);
    \end{tikzpicture}
  \]

  We see that this assignment is contravariantly functorial, arguing that a
  morphism of rings $\Mor{A}{B}$ sends a prime ideal of $B$ to a prime ideal of
...
%%%% TODO: the following constructions are actually giving the Zariski topos of a ring, not the spectrum! \citep{blechschmidt:2017}
...
%   $\Mor[B]{\XTop}{\brk{\RingLoc}}$ together with a homomorphism of rings
...
%   a morphism of topoi $\Mor{\Spec{B}}{\Spec{A}}$ using the universal property
%   of the comma square. First, consider the pasting of the morphism $\Mor{A}{B}$
%   with the 2-cell of the defining comma square for $\Spec{B}$:
%   \[
%     \begin{tikzpicture}[diagram]
%       \node [pullback = <-] (nw) {$\Spec{B}$};
%       \node (ne) [right = 3cm of nw] {$\Pt$};
%       \node (sw) [below = of nw] {$\brk{\RingLoc}$};
%       \node (se) [below = of ne] {$\brk{\Ring}$};
%       \path[->] (nw) edge coordinate [xshift=4mm] (StrB) node [left] {$\Str{\Spec{B}}$} (sw);
...
%         width = 2.5cm,
%       }
%       \node (nww) [above left = of nw] {$\Spec{B}$};
%       \path[->,bend right = 30] (nww) edge coordinate [xshift = 1mm, yshift = 1mm] (l-ctr) node [left] {$\Str{\Spec{B}}$} (sw);
%       \path[->,bend left = 30] (nww) edge coordinate [xshift = -1mm, yshift = -1mm] (r-ctr) (ne);
%       \path[->] (nww) edge (nw);
...
%We will write $\AFF = \OpCat{\Ring}$ for the category of \emph{affine
