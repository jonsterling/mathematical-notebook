...
\begin{para}
  ...
  finite subset of $A$ that generates the unit ideal, then the corresponding family of
...
\begin{para}\label{para:zariski-local-system}
...
\begin{para}
  ...
  covering sieve} is a Zariski dense monomorphism of the form
  $\Mor|>->|{U}{\FPts{\Spec{A}}}$. The dense monomorphisms are maps that will
...
\begin{para}
  %
  A presheaf $X$ is called a \emph{space} when it is right orthogonal to any
  Zariski dense monomorphism, or (without loss of generality) any Zariski
  covering sieve $\Mor|>->|{U}{\FPts{\Spec{A}}}$.
  %
  \[
    \begin{tikzpicture}[diagram]
      \SpliceDiagramSquare{
  ...
  subcategory of $\FunPreSp$ spanned by spaces. 
  $\FunSp$ can be seen to be a
  logos because the class of Zariski local covers is a system of local covers
  (\cref{para:zariski-local-system}) and stable under base change.
  %
\end{para}
