...
\NewDocumentCommand\Functions{s}{\IfBooleanTF{#1}{\dot{\mathsf{F}}}{\mathsf{F}}}
...
\begin{para}
  ...
  representable object in $\FunPreSp$ as an affine scheme; given any presheaf
  $X:\FunPreSp$, a \emph{function on $X$} is a natural transformation
  $\Mor{X}{\Functions}$.
  %
\end{para}
...
%
  Hence, in the logos $\FunPreSp$ of presheaves, one may speak of the presheaf
  $\Functions := \FPts{\Spec{\ZZ\brk{x}}}$ as the \emph{classifying space} for
  functions. 
\end{para}

\begin{para}
  %
  By the same token, we may define a classifying space of units $\Functions* :=
  \FPts{\Spec{\ZZ\brk{x,\Inv{x}}}}$, and the evident open immersion
  $\Mor{\Functions*}{\Functions}$ corresponding under duality to the
...
%
  ...
  arises as a pullback of $\Mor{\Functions*}{\Functions}$:
  \[
    \DiagramSquare{
      nw/style = pullback,
      nw = X_f,
...
\begin{para}\label{para:gen-zar-open-cover}
  %
...
\begin{para}
%
  The generating Zariski open covers (\cref{para:gen-zar-open-cover}) have an
  avatar in the functorial language of $\FunPreSp$, as any maps of the form
  $\Mor{\Coprod{I}{X_\bullet}}{X}$ where $I$ is a finite set, $X$ is an affine
  scheme and each map $\Mor{X_i}{X}$ arises as a pullback of
  $\Mor{\Functions*}{\Functions}$ such that the characteristic functions $\Mor[f_i]{X}{\Functions}$ generate the unit ideal:
  \[
    \DiagramSquare{
      nw/style = pullback,
      nw = X_i,
...
\begin{para}
  ...
  scheme $\FPts{\Spec{B}} : \FunPreSp$ is a functorial space --- this follows
  almost immediately from \nscref{partition-of-unity/para}, because the gluing
