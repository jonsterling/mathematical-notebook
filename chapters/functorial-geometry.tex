...
\begin{para}
  ...
  $\Zar{\Spec{\mathbb{Z}}}$, the classifier of local rings, as a target for the
  functor of points. This is not a good idea, because the full category of
  schemes does not embed fully faithfully into $\Sh{\Zar{\Spec{\mathbb{Z}}}}$
  ...
  The downside of the correct choice for the logos of algebraic spaces is that
...
\begin{para}[Todo]
  \begin{enumerate}
    \item Go back and define Grothendieck universes $\mathscr{U}$, relativizing everything. Some discussion of enlargement for locally presentable categories \`a la \citet{shulman:blog:enlargement} might be called for.
    \item Define the ``$\mathscr{U}_\beta$-logos of algebraic spaces'' to be a localization of $\Hom{\Ring_{\alpha}}{\SET_\beta}$.
  \end{enumerate}
\end{para}
