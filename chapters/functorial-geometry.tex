...
\begin{para}\label{para:zariski-local-cover}
  %
...
\begin{para}
  ...
  $\Mor|>->|{U}{\Yo{A}}$. The dense monomorphisms are maps that will
...
\begin{para}\label{para:functorial-space}
  %
  ...
  by \cref{para:zariski-local-system}.
  %
\end{para}
...
\begin{para}
  ...
  unique section of $X$ at $A$ that restricts to each $s_i$.

  We choose a covering sieve $U\subseteq\Yo{A}$ generated by our local cover as follows:
  \[
    U\prn{B} = \brc{
      \Mor[f]{A}{B} : \Ring
      \vertsep
      \text{$f$ factors through some the localization $\Mor|->>|{A}{A\brk{\Inv{a_i}}}$}
    }
  \]

  We may then define an element $\Mor{U}{X}$; fixing a ring map
  $\Mor[f = b\circ e_i]{A}{B}$ where $\Mor|->>|[e_i]{A}{A\brk{\Inv{a_i}}}$ and
  $\Mor[b]{A\brk{\Inv{a_i}}}{B}$, we choose the section $X\prn{b}\prn{s_i}$ of
  $X$ at $B$. By \cref{para:functorial-space}, we consequently have a morphism
  $\Mor{\Yo{A}}{X}$ uniquely factoring the element $\Mor{U}{X}$,
  and by the Yoneda lemma a corresponding section $\tilde{s}\in X\prn{A}$. 

  We must still argue that $\tilde{s}$ is unique in the sense of the equalizer
  diagram; this follows from the fact that any other section would determine an
  element $\Mor{\Yo{A}}{X}$ that also factors $\Mor{U}{X}$.
\end{para}
